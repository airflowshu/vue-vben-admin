...
watch(
  ...
  (count) => {
    // 获取当前表单的类型值
    const currentType = formApi.values?.type;
  ...
  onOpenChange: async (open) => {
    if (open) {
      ...
      if (drawerData.value.mode === 'edit' && drawerData.value.record) {
        ...
        // 加载现有成员
        try {
           const members = await getKnowledgeBaseMembers(drawerData.value.record.id);
           if (members.data) {
  ...
  >
    <div class="flex flex-col">
      ...
      <div class="flex items-start px-5 pb-5">
        ...
        <div class="w-[100px] flex-shrink-0 text-right pr-4 pt-[5px]">
          <div class="flex items-center justify-end gap-1">
             <span class="text-gray-700 after:content-[':'] after:ml-0.5 after:relative after:top-[-0.5px]">协作者</span>
             <Tooltip title="配置协作者">
                <IconifyIcon 
                  icon="mdi:cog" 
                  class="cursor-pointer text-gray-400 hover:text-primary transition-colors text-base" 
        ...
        <div class="flex-1">
           <div 
             class="flex flex-wrap gap-2 min-h-[32px] p-1 bg-white rounded border border-[#d9d9d9] hover:border-primary transition-colors cursor-pointer"
             @click="openCollabModal"
           >
              ...
              <div v-else class="flex items-center px-2 text-gray-400 text-sm h-[22px]">
                 点击添加协作者
              </div>
           </div>
        </div>
